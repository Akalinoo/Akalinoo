# Hi, I'm Talha

Computer Science student at Ankara University  
Passionate about UI/UX Design, Android Development and Machine Learning
Always learning & building projects to improve my skills  

---

## Featured Projects
- [**Pomopix**](https://github.com/Akalinoo/pomopix) – Kotlin & Jetpack Compose Pomodoro Timer with Foreground Service, customizable work/break durations, and notification support.  
- [**Kaykay Education Site**](https://github.com/Akalinoo/kaykay-egitim-sitesi) – ASP.NET WebForms project with MySQL backend and master page navigation.  
- [**Music App UI**](https://github.com/Akalinoo/music-app-ui) – Clean UI concept for a music application designed with Figma 

---

## Tech Stack
- **Languages:** Kotlin, Python, C#, C++, SQL   
- **Frameworks & Tools:** Jetpack Compose, Android Studio, ASP.NET
- **Other:** Git, GitHub, Linux

---

## Connect with Me
- LinkedIn: https://www.linkedin.com/in/talha-akalin/
- GitHub: https://github.com/Akalinoo
- Email: bytlh06@windowslive.com
